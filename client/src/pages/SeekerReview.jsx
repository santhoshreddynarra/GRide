        ...
        {[1, 2, 3, 4, 5].map((star) => (
          ...
          >
            <Star
              ...
              strokeWidth={(hover || value) >= star ? 1.5 : 1.5}
            />
          </button>
        ))}
      </div>
...
export default function SeekerReview() {
  const { historyId } = useParams(); // historyId is actually the jobId
  ...
  // Fetch job/history detail
  useEffect(() => {
    const fetchDetail = async () => {
      setLoading(true);
      setError("");
      try {
        const res = await axios.get(`/api/seeker/history/${historyId}`, {
          headers: { Authorization: `Bearer ${getToken()}` },
        });
        const h = res.data.history;

        if (h.seekerReviewSubmitted) {
        ...
        if (err.response?.status === 403) { navigate("/seeker/history"); return; }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "/api/reviews",
        { jobId: historyId, rating, comment: comment.trim() },
        { headers: { Authorization: `Bearer ${getToken()}` } }
      );
      setSubmitted(true);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)" }}>
        <div className="flex flex-col items-center gap-4">
          <div className="w-14 h-14 rounded-full border-4 border-blue-200 border-t-blue-600 animate-spin" />
          <p className="text-blue-700 font-semibold">Loading details…</p>
        </div>
      </div>
    );
  }
  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6" style={{ background: "linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)" }}>
        <div className="bg-white rounded-3xl shadow-2xl p-10 max-w-sm w-full text-center animate-in zoom-in-95 duration-300">
          ...
          <p className="text-lg font-black text-gray-800 mb-8">{job?.provider?.name || "the provider"}</p>
          <button
            onClick={() => navigate("/seeker/history")}
            className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-2xl transition-colors shadow-lg shadow-blue-100 active:scale-95"
          >
            Back to History
          </button>
        </div>
  ...
  if (error && (!job || job.seekerReviewSubmitted || job.status !== "Completed")) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6" style={{ background: "linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)" }}>
        <div className="bg-white rounded-3xl shadow-xl p-10 max-w-sm w-full text-center">
          <AlertCircle size={52} className="text-blue-400 mx-auto mb-4" />
          <h2 className="text-xl font-black text-gray-900 mb-2">Cannot Review</h2>
          <p className="text-gray-500 mb-6">{error}</p>
          <button
            onClick={() => navigate("/seeker/history")}
            className="w-full py-3 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-700 transition-colors"
          >
            Back to History
          </button>
        </div>
    ...
    <div className="min-h-screen" style={{ background: "linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)" }}>
      ...
      <nav className="bg-white/80 backdrop-blur-md border-b border-blue-100 sticky top-0 z-50">
        <div className="max-w-2xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              <ArrowLeft size={17} /> History
            </button>
            <span className="text-gray-300 text-lg">›</span>
            <span className="font-black text-blue-700 text-base">Write Review</span>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="slide-up">
          <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-700 px-4 py-2 rounded-full text-xs font-black mb-4 uppercase tracking-wider">
            <Star size={13} /> Leave a Review
          </div>
          <h1 className="text-3xl font-black text-gray-900">How was the job?</h1>
          <p className="text-gray-500 mt-1 font-medium">Your honest feedback helps the community.</p>
        </div>

        {/* Job summary card */}
        {job && (
          <div className="bg-white rounded-3xl border border-blue-100 p-6 shadow-sm slide-up">
            <p className="text-xs font-black text-blue-500 uppercase tracking-widest mb-3">Job Summary</p>
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 shrink-0">
                <Briefcase size={22} />
              </div>
              <div className="flex-1">
                <h2 className="font-black text-xl text-gray-900">{job.serviceTitle}</h2>
                {job.provider && (
                  <p className="flex items-center gap-1.5 text-sm text-gray-500 mt-1">
                    <UserIcon size={13} /> {job.provider.name}
                  </p>
                )}
        ...
        {/* Review form */}
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl border border-blue-100 shadow-sm overflow-hidden slide-up">
          {/* Blue header */}
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-7 py-6">
            ...
            <h3 className="text-xl font-black text-white">
              Rate {job?.provider?.name || "the provider"}
            </h3>
          </div>
          ...
          <div className="p-7 space-y-7">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={`Share your experience working with ${job?.provider?.name || "this provider"}…`}
                rows={5}
                maxLength={1000}
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all font-medium"
              />
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                className="flex-1 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-2xl transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-blue-100"
              >
                {submitting ? (
                  <>
